  ...
  <form action="<?php echo url_for('timetable_assign_classroom') ?>" method="POST" class="assign-classroom">
    <table class="kt-table fees-categories-table">
      ...
      foreach ($searchedStudents as $student) {
        $sectionNo = '<span style="color: #ff0000">Not Assigned</span>';
        if($student['StudentClassroom'][0]['section_no'] == '1') {
          $sectionNo = 'I';
        } else if($student['StudentClassroom'][0]['section_no'] == '2') {
          $sectionNo = 'II';
        }
        ?>
        <tr>
          ...
          <td class="center-align"><?php echo $sectionNo; ?></td>
        </tr>
        <?php
        $counter++;
      }
